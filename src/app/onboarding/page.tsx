...
export default function Onboarding() {
  ...
  const p = passport || { position: 1247, ownerName: "Reviewer" } as any;
      ...
      {step === 0 && (
        <FadeIn className="min-h-[100dvh] flex flex-col items-center justify-center px-6 text-center">
          ...
          <h1 className="text-hero mt-3">
            #<CountUp value={p.position ?? 1247} />
          </h1>
          <p className="text-lead text-ink-muted mt-4 max-w-xs">
            {locale === "vi"
              ? `Chào ${p.ownerName ?? "bạn"}! Bạn là người thứ ${p.position} cùng khám phá Đà Nẵng.`
              : `Welcome ${p.ownerName ?? "explorer"}! You're the ${p.position}th to begin this journey.`}
          </p>
          <button
            onClick={() => setStep(1)}
